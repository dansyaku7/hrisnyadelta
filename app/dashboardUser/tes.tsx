...
import {
  Paper, Table, TableHead, TableBody, TableRow, TableCell, TableContainer, Chip, Alert, CircularProgress
} from "@mui/material";
...
export default function DashboardUser() {
  ...
  useEffect(() => {
    ...
    async function fetchAll() {
      setLoading(true); // Pastikan loading true saat mulai
      try {
        // 1. Fetch User & Pengumuman Paralel
        ...
        // 2. Set Department ID dari User Session
        const userDeptId = userRes?.departmentId?.toString() || "";
        setDepartmentId(userDeptId);
        
        // 3. Set Data Pengumuman
        // Handle format response yang mungkin berbeda (misal { data: [...] } atau langsung [...])
        const annList = Array.isArray(annRes.data) ? annRes.data : (Array.isArray(annRes) ? annRes : []);
        setData(annList);

        // 4. Fetch Detail Employee (Jika ada ID) untuk cek kelengkapan profil & nama department
        ...
        if (userRes?.employeeId) {
          try {
            ...
            // Ambil nama department jika tersedia di data employee
            if (emp?.department) {
                currentDepartment = emp.department;
            } 
            // Fallback: Jika tidak ada nama dept di employee, bisa fetch ke /api/department (opsional)
            // Tapi biasanya sudah di-join atau disimpan di table employee
            ...
            if (
              ...
              !emp?.phone ||
              !emp?.norek ||
              !emp?.atasNama || // Pastikan field ini sesuai database (atas_nama / atasNama)
              !emp?.address
            ) {
        ...
        setLoading(false); // Selesai loading (baik sukses/gagal)
      }
    }
      ...
      <div className="relative z-10 px-6 pt-12 max-w-lg mx-auto min-h-screen flex flex-col">
        ...
        <div className="bg-gradient-to-br from-[#07ABE8] to-[#0047AD] rounded-[35px] p-6 text-white shadow-[0_7px_18px_-5px_rgba(38,166,154,0.5)] mb-6 relative overflow-hidden group transform hover:scale-[1.01] transition-transform duration-300">
          {loading ? (
           ...
           <div className="relative z-10 flex justify-between items-start mb-4 border-b border-white/10 pb-4">
              ...
              <div className="bg-white p-1.5 rounded-2xl shadow-lg shadow-green-900/20 cursor-pointer flex items-center justify-center h-13 w-20 group">
                 <img 
           ...
           <div className="relative z-10 flex justify-between items-end">
              <div>
                 ...
                 <p className="text-xs text-green-100 mt-1 pl-1">Waktu Indonesia Barat</p>
              </div>
        ...
        <div className="mb-4">
            ...
            <div className="flex items-center justify-between mb-3 px-2">
                <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider" style={{ fontFamily: '"Comfortaa", sans-serif' }}>
                    Pengumuman
                </h3>
            </div>

            {/* List Pengumuman (Card Style) */}
            <div className="bg-white rounded-[30px] shadow-xl shadow-gray-100 p-5 min-h-[200px] relative border border-gray-50">
                {loading ? (
                    <div className="flex flex-col items-center justify-center h-40 text-gray-400 gap-3">
                        <CircularProgress size={24} color="success" />
                        <span className="text-xs font-medium">Memuat...</span>
                    </div>
                ) : data.length === 0 ? ( // Menggunakan state 'data' (karena di dashboard logic aslinya pakai data, bukan validAnnouncements secara terpisah di state)
                    <div className="flex flex-col items-center justify-center h-40 text-gray-400 gap-2">
                        <span className="text-3xl grayscale opacity-30">📢</span>
                        <span className="text-xs">Tidak ada pengumuman baru.</span>
                    </div>
                ) : (
                        ...
                        {validAnnouncements.map((ann, index) => (
                            ...
                            >
                                ...
                                <div className="col-span-4 text-center flex justify-center items-center">
                                    {/* Pastikan fungsi priorityColor mereturn elemen JSX atau className yang sesuai */}
